...
function drawPolygon(ctx, polygon, color = '#2196F3', fillColor = 'rgba(33, 150, 243, 0.2)') {
    ctx.beginPath();
...
function drawRotationPath(ctx, polygon, center, color = 'rgba(255, 152, 0, 0.3)') {
    for (const vertex of polygon) {
        ...
        for (let i = 1; i <= numPoints; i++) {
            ...
            ctx.fillStyle = i === numPoints / 4 ? 'rgba(255, 100, 0, 0.7)' : 'rgba(255, 152, 0, 0.5)';
            ctx.fill();
        }
    }
}
...
function visualizePolygon(testCase, containerId = 'testContainer') {
    ...
    const scale = Math.min((canvas.width - 100) / polygonWidth, (canvas.height - 100) / polygonHeight);
    ...
    try {
        ...
        if (result.castable) {
            resultInfo.innerHTML = `<span class="castable">✓ Castable</span> - Rotation center: (${result.center[0].toFixed(2)}, ${result.center[1].toFixed(2)})`;
            
            // Scale the top facet
            const scaledTopFacet = [
                [result.topFacet[0][0] * scale + offsetX, result.topFacet[0][1] * scale + offsetY],
                [result.topFacet[1][0] * scale + offsetX, result.topFacet[1][1] * scale + offsetY]
            ];
            ...
            const scaledCenter = [result.center[0] * scale + offsetX, result.center[1] * scale + offsetY];
...
function generateRandomNonConvexPolygon(numVertices, minSize = 50, maxSize = 150) {
    // First generate a convex polygon
    const convexPolygon = generateRandomConvexPolygon(numVertices, minSize, maxSize);
    ...
    const numToModify = Math.max(1, Math.floor(Math.random() * (numVertices / 2)));
    ...
    for (const index of indicesToModify) {
        ...
        const magnitude = Math.sqrt(vectorToCentroid[0]**2 + vectorToCentroid[1]**2);
        const normalizedVector = [vectorToCentroid[0]/magnitude, vectorToCentroid[1]/magnitude];
...
function generateRandomTestCases(count = 5, minVertices = 5, maxVertices = 10) {
    ...
    for (let i = 0; i < count; i++) {
        const numVertices = minVertices + Math.floor(Math.random() * (maxVertices - minVertices + 1));
...
const predefinedTestCases = [
    ...
    {
        name: "Star Shape",
        polygon: [[3, 0], [4, 2], [6, 2], [5, 3], [6, 5], [3, 4], [0, 5], [1, 3], [0, 2], [2, 2]]
    },
    {
        name: "U-Shape",
    ...
    {
        name: "Arrow Shape",
        polygon: [[0, 0], [2, 0], [3, 1], [4, 0], [6, 0], [3, 3]]
    }
];

// Combine predefined and random test cases
let testCases = [...predefinedTestCases];